...
describe('Cart Integration Flow', () => {
  ...
  describe('Cart Total Calculations', () => {
    test('should calculate total correctly for mixed cart', () => {
      addToCart(1, 2); // Product 1: 2000 * 2 = 4000 (offer price)
      addToCart(2, 1); // Product 2: 1500 * 1 = 1500
      addToCart(3, 3); // Product 3: 2500 * 3 = 7500 (offer price)
  ...
  describe('Error Handling', () => {
    ...
    test('should handle invalid quantities', () => {
      // Test with quantity 0 - should add to cart with quantity 0
      const result1 = addToCart(1, 0);
      expect(result1).toBeDefined();
      expect(result1.length).toBe(1);
      expect(result1[0].quantity).toBe(0);
      
      // Test with negative quantity - should add to cart with negative quantity
      const result2 = addToCart(1, -1);
      expect(result2).toBeDefined();
      expect(result2.length).toBe(1);
      expect(result2[0].quantity).toBe(-1);
