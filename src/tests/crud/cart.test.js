...
describe('Cart CRUD Operations', () => {
  ...
  describe('Cart Total Calculations', () => {
    test('should calculate total for single item', () => {
      addToCart(1, 2); // Product 1 has offer price 2000, quantity 2 = 4000
      const total = getCartTotal();
      expect(total).toBe(4000);
    });

    test('should calculate total for multiple items', () => {
      addToCart(1, 2); // 2000 * 2 = 4000 (offer price)
    ...
    test('should update total after quantity changes', () => {
      ...
      expect(total).toBe(4000); // 2000 * 2 = 4000 (offer price)
      
      updateCartItem(1, 4);
      total = getCartTotal();
      expect(total).toBe(8000); // 2000 * 4 = 8000
    });
  });
